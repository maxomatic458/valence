...
//         // when the window is split, we can only access the main slots of
// player's         // inventory
...
//             if !s.stack.is_empty() {
//                 let max_stack_size =
// s.stack.item.max_stack().max(s.stack.count);                 if
// !(1..=max_stack_size).contains(&(s.stack.count)) {                     return
// false;                 }
//             }
...
//         let max_stack_size =
// carried_item.item.max_stack().max(carried_item.count);         ensure!(
...
// packet.slot_idx == -999,                 "invalid slot index"
//             )
//         }
...
//             )
//         }
//         ClickMode::DoubleClick => ensure!(packet.button == 0, "invalid
// button"),     }
...
//                 // Clicked outside the window, so the client is dropping an
// item                 ensure!(
...
//                 let count_deltas = calculate_net_item_delta(packet, &window,
// cursor_item);                 let expected_delta = match packet.button {
//                     1 => -1,
//                     0 => {
...
//                 let old_slot = window.slot(packet.slot_changes[0].idx as
// u16);                 // TODO: make sure NBT is the same.
//                 //       Sometimes, the client will add nbt data to an item
// if it's missing,                 // like       "Damage" to a sword.
//                 let should_swap: bool = packet.button == 0
//                     && match (!old_slot.is_empty(), !cursor_item.is_empty())
// {                         (true, true) => old_slot.item != cursor_item.item,
//                         (true, false) => true,
//                         (false, true) => cursor_item.count <=
// cursor_item.item.max_stack(),                         (false, false) =>
// false,                     };

//                 if should_swap {
//                     // assert that a swap occurs
//                     ensure!(
//                         // There are some cases where the client will add NBT
...
//                     let count_deltas = calculate_net_item_delta(packet,
// &window, cursor_item);                     ensure!(
//                         count_deltas == 0,
...
//             );
//             ensure!(
//                 packet.slot_idx == packet.slot_changes.first().map_or(-2, |s|
// s.idx),                 "slot index does not match modified slot"
//             );

//             let old_slot = window.slot(packet.slot_idx as u16);
//             let new_slot = &packet.slot_changes[0].stack;
//             let is_transmuting = match (!old_slot.is_empty(),
// !new_slot.is_empty()) {                 // TODO: make sure NBT is the same.
//                 // Sometimes, the client will add nbt data to an item if it's
// missing, like                 // "Damage" to a sword.
...
//                 );
//             } else {
//                 ensure!(packet.slot_changes.is_empty() && packet.carried_item
// == cursor_item.0);             }
//         }
...
// /// Calculate the total difference in item counts if the changes in this
// packet /// were to be applied.
// ///
// /// Returns a positive number if items were added to the window, and a
// negative /// number if items were removed from the window.
...
// {             (true, true) => i32::from(new_slot.count) -
// i32::from(old_slot.count),             (true, false) =>
// -i32::from(old_slot.count),             (false, true) =>
// i32::from(new_slot.count),             (false, false) => 0,
//         };
//     }

//     net_item_delta += match (!cursor_item.is_empty(),
// !packet.carried_item.is_empty()) {         (true, true) =>
// i32::from(packet.carried_item.count) - i32::from(cursor_item.count),
...
//         let window = InventoryWindow::new(&player_inventory,
// Some(&inventory));         let cursor_item =
// CursorItem(ItemStack::new(ItemKind::Diamond, 64));
...
//         let inventory = Inventory::new(InventoryKind::Generic9x1);
//         let window = InventoryWindow::new(&player_inventory,
// Some(&inventory));         let cursor_item = CursorItem::default();
...
//         validate_click_slot_packet(&packet, &player_inventory,
// Some(&inventory), &cursor_item)             .expect("packet should be
// valid");     }
...
//                 .with_components(vec![ItemComponent::Damage { damage:
// 1.into() }]),         };
...
//         validate_click_slot_packet(&packet1, &player_inventory,
// Some(&inventory1), &cursor_item)             .expect("packet should be
// valid");

//         validate_click_slot_packet(&packet2, &player_inventory,
// Some(&inventory2), &cursor_item)             .expect("packet should be
// valid");     }
...
//         validate_click_slot_packet(&packet1, &player_inventory,
// Some(&inventory1), &cursor_item)             .expect_err("packet 1 should
// fail item duplication check");

//         validate_click_slot_packet(&packet2, &player_inventory,
// Some(&inventory2), &cursor_item)             .expect_err("packet 2 should
// fail item duplication check");

//         validate_click_slot_packet(&packet3, &player_inventory,
// Some(&inventory1), &cursor_item)             .expect_err("packet 3 should
// fail item duplication check");     }
...
//             validate_click_slot_packet(packet, &player_inventory, None,
// &cursor_item).expect_err(                 &format!("packet {i} passed item
// duplication check when it should have failed"),             );
//         }
//     }
