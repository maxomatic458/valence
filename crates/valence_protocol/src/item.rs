...
pub use valence_generated::item::{ItemKind, SerItemComponent, SerAttributeModifier};
        ...
        match serialized {
            SerItemComponent::Enchantments { levels } => {
                ItemComponent::Enchantments {
                    enchantments: levels
                        .into_iter()
                        .map(|(id, level)| (VarInt(id), VarInt(level)))
                        .collect(),
                }
            },
            ...
            SerItemComponent::ItemModel(model) => ItemComponent::ItemModel { model: Ident::new(model).unwrap().into() },
            SerItemComponent::Rarity(rarity) => ItemComponent::Rarity { rarity: Rarity::from_str(rarity).unwrap() }, // We can safely unwrap because SerItemComponent is generated by the build script
            SerItemComponent::ItemName(name) => ItemComponent::ItemName { name: Text::from(name) },
            SerItemComponent::Damage(damage) => ItemComponent::Damage { damage: VarInt(damage) },
            SerItemComponent::MaxDamage(max_damage) => ItemComponent::MaxDamage { max_damage: VarInt(max_damage) },
            SerItemComponent::Repairable { items } => ItemComponent::Repairable { items:  IDSet::NamedSet(items.to_string())},
        }
    }
}
